      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden divide-y divide-slate-100">
        {notifications.length === 0 ? (
          ...
          notifications.map(notif => (
            <div key={notif._id} onClick={() => markAsRead(notif._id, notif.read)} className={`p-4 flex items-start gap-4 transition-colors hover:bg-slate-50 cursor-pointer ${!notif.read ? 'bg-brand-50/30' : ''}`}>
              <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-brand-500 bg-brand-50">
                <Bell size={20} />
              </div>
